...
import { getProvider, type ChatMessage } from '../llm/providers.js';

export async function planNext(args: {
  goal: string;
  observation: { url: string; textSnippet?: string };
  history: ChatMessage[];
}): Promise<{ plan: PlanResponse; history: ChatMessage[] }> {
...
`;
  ...
  const messages: ChatMessage[] = [...args.history, { role: 'user', content: user }];
